...
function isMenuExpanded($routes) {
    return isActiveRoute($routes) ? 'true' : 'false';
}
...
function get_youtube_video_id($url) {
    ...
    // Handle short URLs (e.g., https://youtu.be/dQw4w9WgXcQ)
    if (isset($parsedUrl['host']) && ($parsedUrl['host'] == 'youtu.be')) {
        return ltrim($parsedUrl['path'], '/');
    }

    // Handle standard URLs (e.g., https://www.youtube.com/watch?v=dQw4w9WgXcQ)
...
function truncateHtml($html, $limit, $ending = '...') {
    $totalLength = 0;
    $openTags = [];
    $output = '';
    ...
    foreach ($matches as $match) {
        if (!empty($match[1])) {
            ...
            if (preg_match('/^<(\w+)[^>]*>$/s', $match[1], $tagMatch)) {
                $tag = $tagMatch[1];
                if (!in_array($tag, $selfClosingTags)) {
                    $openTags[] = $tag;
                }
            } elseif (preg_match('/^<\/(\w+)>$/s', $match[1], $tagMatch)) {
                $tag = $tagMatch[1];
                $pos = array_search($tag, array_reverse($openTags, true));
                if ($pos !== false) {
                    unset($openTags[count($openTags) - 1 - $pos]);
                }
            }
            $output .= $match[1];
        }
        ...
        // Handle text content
        $contentLength = mb_strlen($match[2]);
        if ($totalLength + $contentLength > $limit) {
            $output .= mb_substr($match[2], 0, $limit - $totalLength) . $ending;
            break;
        } else {
            $output .= $match[2];
